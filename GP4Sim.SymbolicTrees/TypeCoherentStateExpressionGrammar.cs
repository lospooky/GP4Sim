...
{
    [StorableClass]
    [Item("TypeCoherentStateExpressionGrammar", "Represents a grammar for functional expressions in which special syntactic constraints are enforced so that boolean and real-valued expressions are not mixed.")]
    public class TypeCoherentStateExpressionGrammar : SymbolicExpressionGrammar, ISymbolicDataAnalysisGrammar
    {
        ...
        [StorableConstructor]
        protected TypeCoherentStateExpressionGrammar(bool deserializing) : base(deserializing) { }
        protected TypeCoherentStateExpressionGrammar(TypeCoherentStateExpressionGrammar original, Cloner cloner) : base(original, cloner) { }
        public TypeCoherentStateExpressionGrammar()
            : base(ItemAttribute.GetName(typeof(TypeCoherentStateExpressionGrammar)), ItemAttribute.GetDescription(typeof(TypeCoherentStateExpressionGrammar)))
        {
            Initialize();
        }
        public override IDeepCloneable Clone(Cloner cloner)
        {
            return new TypeCoherentStateExpressionGrammar(this, cloner);
        }
        ...
        private void Initialize()
        {
            ...
            constant.MinValue = -100;
            constant.MaxValue = 100;
            var variableSymbol = new HeuristicLab.Problems.DataAnalysis.Symbolic.Variable();
            var laggedVariableSymbol = new LaggedVariable();
            var stateSymbol = new InternalState();
        ...
        fresnelCosineIntegral,fresnelSineIntegral,gamma,hypCosineIntegral,hypSineIntegral,norm, psi, sineIntegral});
            var terminalSymbols = new GroupSymbol(TerminalsName, new List<ISymbol> { constant, variableSymbol, laggedVariableSymbol, stateSymbol });
            ...
            var conditionSymbols = new GroupSymbol(ConditionsName, new List<ISymbol> { @if });
