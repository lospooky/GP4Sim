...
{
    [View("SolutionList View")]
    [Content(typeof(SolutionList<>), true)]
    public partial class SolutionListView<T> : ItemView where T : class, IDataAnalysisSolution, IItem
    {
                ...
                if (items.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        if ((result & DragDropEffects.Move) == DragDropEffects.Move)
                        {
                            ...
                            {
                                if (listViewItem.Tag != null) Content.RemoveAt(listViewItem.Index);
                            }
                        }
                    }
                }
        ...
        private void ExportAllButton_Click(object sender, EventArgs e)
        {
            ...
            VistaFolderBrowserDialog vfd = new VistaFolderBrowserDialog();
            //ofd.Filter = "C# File (*.cs) | *.cs";
            vfd.ShowNewFolderButton = true;
            DialogResult folderResult = vfd.ShowDialog();
            if (folderResult == DialogResult.OK)
            {
                InputDialog fnd = new InputDialog();
                fnd.WindowTitle = "Batch FileName";
                fnd.MainInstruction = "Insert Base FileName";
                fnd.Content = "The Agent Files will be progressively saved as BaseName_01.cs, BaseName_02.cs, ... in the selected directory";
                fnd.Input = (Content[0] as ITradingSolution).DescriptiveName;
                DialogResult fnResult = fnd.ShowDialog();

                if (fnResult == DialogResult.OK)
                {
                    int idx = 1;
        ...
        private void ExcelAllButton_Click(object sender, EventArgs e)
        {
            VistaFolderBrowserDialog vfd = new VistaFolderBrowserDialog();
            //ofd.Filter = "C# File (*.cs) | *.cs";
            vfd.ShowNewFolderButton = true;
            ...
            if (folderResult == DialogResult.OK)
            {
                InputDialog fnd = new InputDialog();
                fnd.WindowTitle = "Batch FileName";
                fnd.MainInstruction = "Insert FileName";
                fnd.Content = "One Excel report will be created for all the solutions";
                fnd.Input = "Batch Report";
                DialogResult fnResult = fnd.ShowDialog();
                if (fnResult == DialogResult.OK)
                {
                    string fullPath = Path.Combine(vfd.SelectedPath, fnd.Input + ".xlsx");
                    List<ISymbolicDataAnalysisSolution> solutions = Content.Select(x => x as ISymbolicDataAnalysisSolution).ToList();

                    ExcelReportFactory.CreateAndSaveBatchReport(solutions, fullPath);
                }
            }
        }
    }
